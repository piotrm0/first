...
class match_alliance:
	...
	self.raw_score = 3 * self.scores[SCORE_CENTER_GOAL];
	self.raw_score += self.scores[SCORE_FAR_GOAL];
	self.raw_score += self.scores[SCORE_NEAR_GOAL];
	self.rts = self.raw_score;
...
def cc_load_match(cc, number, callback_fn):
    ...
    queries = ("SELECT * FROM game_match" + where,
	       "SELECT * FROM alliance_team" + where,
	       "SELECT * FROM team_score" + where + "AND position = 0");
    cc.query(queries, cc_load_match_done, match);

def cc_load_match_done(results, match = None):
    if (not isinstance(match, game_match) or not hasattr(match, "on_load")
	or not callable(match.on_load)):
	raise Exception("cc_load_match_done called improperly");
    if len(results) < 3:
	gtk_error_alert("<b>Unable to load match:</b>\n\n" +
			"Did not receive 3 results.");
	return;
    if results[0].type != query_result.TYPE_RSET:
	gtk_error_alert("<b>Unable to load match:</b>\n\n" + results[0].msg);
    else:
	if len(results[0].rset) > 0:
	    row = results[0].rset[0];
    ...
    if results[1].type != query_result.TYPE_RSET:
	gtk_error_alert("<b>Unable to load match:</b>\n\n" + results[1].msg);
    else:
    ...
    if results[2].type != query_result.TYPE_RSET:
	gtk_error_alert("<b>Unable to load match:</b>\n\n" + results[2].msg);
    else:
...
def cc_save_match(cc, match, callback_fn):
	...
	"UPDATE game_match SET status_id = " + str(match.match_status) +
		", winner_color_id = " + str(match.winner) + where ];
    for color in COLORS:
	for position in POSITIONS:
	    queries.append("UPDATE alliance_team SET flags = " +
		str(match.alliance[color].team[position].flags) +
		", score = " +
		str(match.alliance[color].team[position].score) +
		", points = " +
		str(match.alliance[color].team[position].points) + where +
		" AND alliance_color_id = " + str(color) +
		" AND position = " + str(position));
	for el in SCORES:
	    queries.append("INSERT INTO team_score (match_level, " +
		"match_number, match_index, alliance_color_id, " +
		"position, score_attribute_id, value) VALUES (" +
		str(match_level) + ", " + str(match_number) + ", " +
		str(match_index) + ", " + str(color) + ", " +
		"0, " + str(el) + ", " +
		str(match.alliance[color].scores[el]) + ")");
    queries.append("COMMIT");
    cc.query(queries, cc_save_match_done, match);

def cc_save_match_teams(cc, match, callback_fn):
    if not callable(callback_fn):
	raise Exception("argument #3 to cc_save_match is not a callback");
    (match_level, match_number, match_index) = match.match_number;
    match.on_save = callback_fn;
    ...
    for color in COLORS:
	for position in POSITIONS:
	    queries.append("UPDATE alliance_team SET team_number = " +
		str(match.alliance[color].team[position].team_number) +
		where + " AND alliance_color_id = " + str(color) +
		" AND position = " + str(position));
    queries.append("COMMIT");
    cc.query(queries, cc_save_match_done, match);

def cc_save_match_done(results, match = None):
    if (not isinstance(match, game_match) or not hasattr(match, "on_save")
      or not callable(match.on_save)):
	raise Exception("cc_save_match_done called improperly");
    for result in results:
	if result.type == query_result.TYPE_ERROR:
	    gtk_alert_error("<b>Unable to save match:</b>\n\n" + result.msg);
	    break;
    match.on_save();
