...
INSERT INTO score_attribute (score_attribute_id, name, description) VALUES (251, 'side_goal', '');
END;
...
CREATE OR REPLACE VIEW participant_results AS
	SELECT	team_number,
		...
		num_matches - wins - losses AS matches_tied,
		cast((2 * wins + losses) as numeric(6,3)) / max(1, num_matches) AS record,
		score_max,
		points_sum,
		cast(points_sum as numeric(6,3)) / max(1, num_matches) AS points_ave
	FROM	(
		SELECT	team_number,
	...
	ORDER BY record DESC, points_ave DESC;
