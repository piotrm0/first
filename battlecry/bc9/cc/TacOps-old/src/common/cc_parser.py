...
class cc_parser:
    ...
    def cant_handle(func, line):
        print "%s: cannot handle [%s] [%s]" % (func, line, line.encode('hex_codec'))
    ...
    def parse_line(self, line):
        ...
        try:
            ...
            print "parse_line: cannot parse line (%s)" % (text)
            return []
...
class rset:
    ...
    def __str__(self):
        return "<rset %s with %s rows %s columns>" % (self.name, len(self.rows), len(self.head))
...
class env:
    ...
    def set_value(self, value):
        ...
        for i in range(len(self.callbacks)):
            if not self.callbacks[i] is None:
                self.callbacks[i](self.key, self.value, value)
