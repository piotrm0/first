...
ui.key_handler = new Object();
ui.key_handler[32] = function() {
    ui.keypress_space();
};
...
key_listen.onKeyDown = function() {
    ...
    if (_root.ui.key_handler.hasOwnProperty(key)) {
	_root.ui.key_handler[key]();
    }
}
...
with(ui.menu_windows) {
  ...
  addMenuItem({label:"Questions", instanceName:"sets"});
  addMenuItem({label:"Question", instanceName:"question"});
  addMenuItem({label:"Spins", instanceName:"final"});
  addMenuItem({label:"Teams", instanceName:"teams"});
  addMenuItem({label:"Media", instanceName:"media"});
...
ui.menu_windows['media'].click = function(obj) {
  ui.click_view_mm(obj);
}
...
with(ui.menu_stages) {
  addMenuItem({label:"None",   instanceName:"none"});
  addMenuItem({label:"Normal", instanceName:"normal"});
  addMenuItem({label:"Final",  instanceName:"final"});
}

ui.menu_stages['none'].click = function(obj) {
  main.stages.set_stage("none");
};
...
ui.menu_sound = ui.menubar.addMenu("Media");
with(ui.menu_sound) {
  addMenuItem({label:"View", instanceName:"view"});
...
ui.menu_sound['view'].click = function(obj) {
    ui.click_view_mm();
};
...
ui.windows['mm'].addEventListener("click", listen_window);
ui.windows['mm']['click'] = function(obj) {
  obj._visible = false;
};

ui.click_save_overlay = function() {
    var w = ui.windows['mm'].content;
    _root.net.send_smod("_root.ui.handle_mod_overlay_color", [w.box_color.text]);
};

ui.click_show_overlay = function(doshow:Boolean) {
    _root.net.send_smod("_root.ui.handle_mod_show_overlay", [doshow]);
};

ui.handle_mod_overlay_color = {mod: function(d:Array) {
    var w = ui.windows['mm'].content;

    w.box_color.text = d[0];

    var temp_color = new Color(_root.main.overlay);
    temp_color.setRGB(Number(d[0]));
}};

ui.handle_mod_show_overlay = {mod: function(d:Array) {
    var w = ui.windows['mm'].content;

    var doshow = text2bool(d[0]);

    w.check_overlay.selected = doshow;

    _root.main.overlay._visible = doshow;
}};

ui.click_view_mm = function(obj) {
  with(ui.windows['mm']) {
    _visible = true;
...
ui.click_play_theme = function(doplay:Boolean) {
    _root.main.mod_theme(doplay);
}

ui.click_play_spin = function(doplay:Boolean) {
    _root.main.mod_spin(doplay);
}

ui.click_play_whammy = function() {
    _root.main.mod_whammy(true);
}

ui.click_play_win = function() {
    _root.main.mod_win(true);
}

ui.click_play_win_big = function() {
    _root.main.mod_win_big(true);
}
...
ui.click_connect = function(obj) {
    _root.ui.windows['connect'].setFocus();
...
ui.handle_mod_view_text = {mod: function(s) {    
    ...
    _root.ui.view_text = s;
    _root.ui.fill_view_text();
}};

ui.fill_view_text = function() {
...
ui.click_save_won = function():Void {
    var w = _root.ui.windows['f'].content;

    _root.net.send_smod("_root.ui.handle_save_won", [Number(w.won_score.text),
						     Number(w.won_spins.text),
						     Number(w.won_whammies.text)]);
}

ui.handle_save_won = {mod: function(d:Array):Void {
    var w = _root.ui.windows['f'].content;

    w.won_score.text    = d[0];
    w.won_spins.text    = d[1];
    w.won_whammies.text = d[2];

    ui.select_won_enables();
    ui.give_spins_enables();
}};

ui.keypress_space = function():Void {
    var f = ui.stage_final;
    var w = _root.ui.windows['f'].content;

    if (_root.net.type == "control") {
	
    } else {
	if (f.is_spinning()) {
	    _root.ui.click_spin_mark();
	}
    }
}

ui.click_spin_mark = function ():Void {
    var w = _root.ui.windows['f'].content;
    var f = ui.stage_final;

    if (! f.is_spinning()) {
	trace("!!! mark pressed when not spinning");
	return;
    }

    var res = f.stop_live();

    _root.net.send_mod("_root.ui.handle_mod_set_spin_state", f.get_state());
    _root.net.send_smod("_root.ui.handle_mod_spin", [2]);
}

ui.handle_mod_set_spin_state = {mod: function(d:Array):Void {
    var f = ui.stage_final;

    var temp = new Array();
    for (var i = 0; i < d.length; i++) {
	temp.push(Number(d[i]));
    }

    f.set_state(temp);
}};

ui.fill_won = function(res:Array) {
    var w = _root.ui.windows['f'].content;
    var f = ui.stage_final;

    var stage = _root.ui.spin_stage;

    var score    = res[0];
    var spins    = res[1];
    var whammies = res[2];

    w.won_score.text    = score;
    w.won_spins.text    = spins;
    w.won_whammies.text = whammies;
    ...
    if ((score    > 0) &&
	(whammies < 0)) {
	w.select_won_whammies.selected = true;
    }

    if (spins > 0) {
	if ((stage > 0) &&
	    (stage < 4)) {
	    w["give_rolls_" + stage].selected = true;
	}
    }

    w.button_score.enabled = true;
}

ui.give_spins_enables = function():Void {
    var w = _root.ui.windows['f'].content;

    var score    = Number(w.won_score.text);
    var spins    = Number(w.won_spins.text);
    var whammies = Number(w.won_whammies.text);

    var show = (spins > 0);

    w.give_rolls_1.enabled = show;
    w.give_rolls_2.enabled = show;
    w.give_rolls_3.enabled = show;

    if ((show) &&
	(_root.ui.spin_stage > 0) &&
	(_root.ui.spin_stage < 4) &&
	(! w.give_rolls_1.selected) &&
	(! w.give_rolls_2.selected) &&
	(! w.give_rolls_3.selected)) {
	w['give_rolls_' + _root.ui.spin_stage].selected = true;
    }

    if (! show) {
	w.give_rolls_1.selected = false;
	w.give_rolls_2.selected = false;
	w.give_rolls_3.selected = false;
    }

}

ui.select_won_enables = function():Void {
    var w = _root.ui.windows['f'].content;
    ...
    if ((score > 0) &&
	(whammies < 0)) {
	w.select_won_score.enabled    = true;
	w.select_won_whammies.enabled = true;
    } else {
	w.select_won_score.enabled    = false;
	w.select_won_whammies.enabled = false;
	w.select_won_score.selected    = false;
	w.select_won_whammies.selected = false;
    }
}

ui.click_spin = function() {
    var f = ui.stage_final;
    var w = _root.ui.windows['f'].content;

    var send_func = net.send_mod;
    
    if (w.check_local.selected) {
	send_func = net.send_smod;
    }

    if ((f.is_spinning()) ||
	(ui.is_spinning)) {
	net.send_smod("_root.ui.handle_mod_spin", [0]);
	ui.is_spinning = false;
    } else {
	send_func("_root.ui.handle_mod_spin", [1]);
	w.button_spin.label = "Stop";
	ui.is_spinning = true;
    }

    //_root.main.mod_spin(_root.ui.is_spinning, true);
}

ui.handle_mod_spin = {mod: function(d:Array) {
    var mode = Number(d[0]);

    var w = _root.ui.windows['f'].content;
    var t = _root.ui.windows['t'].content;
    var f = _root.ui.stage_final;

    var stage = _root.ui.spin_stage;

    if (mode == 1) {
	f.start_spin();
	w.button_spin.enabled = true;
	w.button_mark.enabled = true;
	w.button_spin.label = "Stop";

	if (stage > 0) {
	    var rolls_earned = Number(t.p_rolls_earned[stage].text);
	    var rolls_passed = Number(t.p_rolls_passed[stage].text);

	    if (w.roll_use_earned.selected) {
		rolls_earned--;
	    } else if (w.roll_use_passed.selected) {
		rolls_passed--;
	    } else if (w.roll_use_none.selected) {

	    } else {
		trace("!!! no roll use selected");
	    }

	    t.p_rolls_earned[stage].text = rolls_earned;
	    t.p_rolls_passed[stage].text = rolls_passed;

	    _root.ui.click_save_players();
	}
	...
	w.button_spin.enabled = true;
	w.button_mark.enabled = false;
	w.button_spin.label = "Spin";
...
ui.click_enable_score_final = function() {
  var f = _root.ui.windows['f'].content;
  f.button_score.enabled = true;
}
...
ui.click_score_final = function() {
  ...
  var score        = Number(t.p_score[stage].text);
  var spins_earned = Number(t.p_rolls_earned[stage].text);
  var whammies     = Number(t.p_whammies[stage].text);

  var won_score    = Number(f.won_score.text);
  var won_spins    = Number(f.won_spins.text);
  var won_whammies = Number(f.won_whammies.text);

  if ((won_score > 0) &&
      (won_whammies < 0)) {
      if (f.select_won_score.selected) {
	  score += won_score;
      } else {
	  whammies += won_whammies;
      }
  } else if (won_spins > 0) {
      var target_team = stage;

      if (f.give_rolls_1.selected) { target_team = 1; }
      else if (f.give_rolls_2.selected) { target_team = 2; }
      else if (f.give_rolls_3.selected) { target_team = 3; }
      else {
	  trace("!!! no target team selected")
	  return;
      }

      if (target_team == stage) {
	  spins_earned += won_spins;
      } else {
	  t.p_rolls_passed[target_team].text = Number(t.p_rolls_passed[target_team].text) + won_spins;
      }
      
      score += won_score;
  } else if (won_whammies > 0) {
      whammies += won_whammies;
      score     = 0;
  } else if (won_score > 0) {
      score += won_score;
  } else {
      trace("!!! don't know how to score this (" + won_score + "," + won_spins + "," + won_whammies + ")");
      return;
  }

  t.p_score[stage].text        = score;
  t.p_rolls_earned[stage].text = spins_earned;
  t.p_whammies[stage].text     = whammies;

  ui.click_save_players(); // *** mod players window

  f.button_score.enabled = false;
};
...
ui.spend_enables = function():Void {
    var w = _root.ui.windows['f'].content;
    var t = _root.ui.windows['t'].content;

    var stage = this.spin_stage;

    //trace("spend enables at stage " + stage);

    if ((this.spin_stage == 0) ||
	(this.spin_stage > 3)) {
	w.roll_use_none.selected = true;
	w.roll_use_earned.enabled = false;
	w.roll_use_passed.enabled = false;
	w.roll_use_none.enabled   = false;
    } else {
	var rolls_earned = Number(t.p_rolls_earned[stage].text);
	var rolls_passed = Number(t.p_rolls_passed[stage].text);

	w.roll_use_earned.enabled = (rolls_earned > 0);
	w.roll_use_passed.enabled = (rolls_passed > 0);

	if (rolls_earned == 0) {
	    if (rolls_passed == 0) {
		w.roll_use_none.selected = true;
	    } else if (w.roll_use_earned.selected) {
		w.roll_use_passed.selected = true;
	    }
	} else if ((rolls_passed == 0) &&
		   (w.roll_use_passed.selected)) {
		       w.roll_use_earned.selected = true;
	}
	w.roll_use_none.enabled = true;
    }
}

ui.click_change_final = function(num) {
  net.send_smod("_root.ui.handle_spin_stage", [num]);
}

ui.handle_spin_stage = {mod: function(d:Array) {
    var w = _root.ui.windows['f'].content;
    var t = _root.ui.windows['t'].content;
    var s = _root.main.stages.final;

    var stage = _root.ui.spin_stage = Number(d[0]);
    _root.main.stages.players.set_active(stage);  
    _root.ui.spend_enables();

    w['p' + stage + "_s"].selected = true;

    if (stage == 0) {
	
    } else if (stage < 4) {
	if (Number(t.p_rolls_passed[stage].text) > 0) {
	    w.roll_use_passed.selected = true;
	} else if (Number(t.p_rolls_earned[stage].text) > 0) {
	    w.roll_use_earned.selected = true;
	} else {
	    w.roll_use.none.selected = true;
	}
    }
}};
