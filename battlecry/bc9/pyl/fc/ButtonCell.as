...
class ButtonCell extends UIComponent {
  ...
  public function createChildren(Void) : Void {
    check = createObject("Button", "check", 1, {styleName:this, owner:this});
    check.label = "A";
  ...
  public function size(Void) : Void {
    check.setSize(PREFERRED_WIDTH-4, PREFERRED_WIDTH-4);
...
/* Note:If item is undefined, nothing should be rendered in the cell, 
  ...
  public function click() {
    trace("clicked: " + getCellIndex().itemIndex + " " + getDataLabel());
//		listOwner.dataProvider.editField(getCellIndex().itemIndex, getDataLabel(), check.selected);
    _root.ui.windows['q'].content.click_answer(getCellIndex().itemIndex);
  }
